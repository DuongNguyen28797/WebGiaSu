...
<div class="templatemo-content-widget white-bg">
	...
	<h2 class="text-uppercase margin-bottom-10 text-center" style="margin-bottom: 1%; margin-top: 5%; margin-left: 17%;color: #f1b458" >Gia Sư</h2>
	<div>
	          ...
	          <c:if test="${param['msg'] eq 'add' }">
					  ...
					  <h4 style="color:#1E90FF; margin-left: 23%;"><b>Đã xoá ${param['soluongdel']} Gia Sư</b></h4>
					  </c:if>
					  <c:if test="${param['msg'] eq 'nondel' }">
					  <h4 style="color:#1E90FF; margin-left: 23%;"><b>Xoá thất bại</b></h4>
	   </c:if>
</div></br>
	<form action="${pageContext.request.contextPath}/admin/giasu/dels" method="get">
		<div class="table-responsive">
					...
					<thead>
						<tr class="text-center" role="row" style="text-align: center; height: 100px;">
							...
							<th class="sorting" tabindex="0" aria-controls="myTable"
								...
								aria-label="Tên Gia Sư: activate to sort column ascending">Tên Gia Sư</th>
							...
							<th class="sorting" tabindex="0" aria-controls="myTable"
								rowspan="1" colspan="1" style="width: 40px; text-align: center;"
								aria-label="Trạng thái: activate to sort column ascending">Web Xu</th>	
		   ...
		   <c:forEach items="${listgiasu}" var="obj" >
			      <c:set var="name" value="${obj.fullname}"></c:set>
				  <c:set var="id" value="${obj.id_user}"></c:set>
				  <c:set var="webxu" value="${split.money(obj.webxu)}"></c:set>
				  <c:set var="urlPic" value="${pageContext.request.contextPath}/files/${obj.picture}"></c:set>
					...
					<tr class="odd" role="row" style="text-align: center; height: 40px;">
							...
							<td ><a href="${pageContext.request.contextPath}/admin/giasu/chitiet/${id}">${name}</a></td>
							
							<c:choose>
							   <c:when test="${obj.enable==1}">
                           ...
                           <td class=""><a href="">${webxu}</a> Xu</td>
                           
							<td align="center"><a href="${pageContext.request.contextPath}/admin/giasu/tangxu/${id}">Tăng Web Xu </a> / <a href="${pageContext.request.contextPath}/admin/giasu/del/${id}" onclick="return confirm('Bạn chắc chắn muốn Xoá ?');"> Xóa </a></td>
...
$(function(){
	$(document).on('click','#myTable .edit_active a', function(){
		...
		var $_point = $(this);
		$.ajax({
			url:"${pageContext.request.contextPath}/admin/giasu/kichhoat",
			type: 'POST',
			cache: false,
			data: {
				id_sp: id_news,
