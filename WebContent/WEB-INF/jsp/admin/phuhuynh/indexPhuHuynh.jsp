...
<div class="templatemo-content-widget white-bg">
	...
	<div>
	          ...
	          <c:if test="${param['msg'] eq 'add' }">
					  ...
					  <h4 style="color:#1E90FF; margin-left: 23%;"><b>Đã xoá ${param['soluongdel']} Phụ Huynh</b></h4>
					  </c:if>
					  <c:if test="${param['msg'] eq 'nondel' }">
					  <h4 style="color:#1E90FF; margin-left: 23%;"><b>Xoá thất bại</b></h4>
	   </c:if>
</div></br>
	<form action="${pageContext.request.contextPath}/admin/phuhuynh/dels" method="get">
		<div class="table-responsive">
					...
					<thead>
						<tr class="text-center" role="row" style="text-align: center; height: 100px;">
							...
							<th class="sorting" tabindex="0" aria-controls="myTable"
								...
								aria-label="Tên Gia Sư: activate to sort column ascending">Tên Phụ Huynh</th>
		   ...
		   <c:forEach items="${listphuhuynh}" var="obj" >
				  ...
				  <c:set var="urlPic" value="${pageContext.request.contextPath}/files/${obj.picture}"></c:set>
					...
					<tr class="odd" role="row" style="text-align: center; height: 40px;">
							...
							<td ><a href="${pageContext.request.contextPath}/admin/phuhuynh/chitiet-phuhuynh/${id}">${name}</a></td>
                           ...
                           <td class=""><a href="">${webxu}</a> Xu</td>
							...
							<td align="center"><a href="${pageContext.request.contextPath}/admin/phuhuynh/tangxu/${id}">Tăng Web Xu </a> / <a href="${pageContext.request.contextPath}/admin/phuhuynh/del/${id}" onclick="return confirm('Bạn chắc chắn muốn Xoá ?');"> Xóa </a></td>
...
$(function(){
	$(document).on('click','#myTable .edit_active a', function(){
		...
		var $_point = $(this);
		$.ajax({
			url:"${pageContext.request.contextPath}/admin/phuhuynh/kichhoat",
			type: 'POST',
			cache: false,
			data: {
				id_sp: id_news,
